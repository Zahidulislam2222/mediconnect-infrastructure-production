...
# Note: SecureString values use placeholder — actual values preserved via lifecycle ignore

locals {
  ssm_us_string_params = {
...
resource "aws_ssm_parameter" "us_string" {
  provider = aws.us
  for_each = local.ssm_us_string_params
...
resource "aws_ssm_parameter" "us_secure" {
  provider = aws.us
  for_each = local.ssm_us_secure_params
