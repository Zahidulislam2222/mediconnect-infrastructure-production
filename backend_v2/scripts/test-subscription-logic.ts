import { PlanId, DoctorTier, calculateDiscountedPrice, calculateShares, isDoctorRateIncreaseAllowed, isGpSpecialty } from '../shared/subscription';
...
const free = calculateDiscountedPrice(200, PlanId.FREE);
check(free.discountedPrice === 200, 'Free: $200 → $200 (no discount)');
check(free.savings === 0, 'Free: $0 savings');

const plus = calculateDiscountedPrice(200, PlanId.PLUS);
...
const premium = calculateDiscountedPrice(200, PlanId.PREMIUM);
...
const cheap = calculateDiscountedPrice(50, PlanId.PLUS);
check(cheap.discountedPrice === 40, `Plus $50 → $${cheap.discountedPrice} (expected $40)`);

console.log('\n=== Revenue Shares ===');
const newDoc = calculateShares(160, DoctorTier.NEW);
...
const est = calculateShares(160, DoctorTier.ESTABLISHED);
check(est.doctorShare === 136, `Established $160: gets $${est.doctorShare} (expected $136)`);

const top = calculateShares(160, DoctorTier.TOP);
...
for (const plan of [PlanId.PLUS, PlanId.PREMIUM] as const) {
    for (const tier of [DoctorTier.NEW, DoctorTier.ESTABLISHED, DoctorTier.TOP] as const) {
